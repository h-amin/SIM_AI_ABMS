globals [
  selected-car ; huidige gekozen auto
  lanes        ; lijst van y coordinaten voor verschillende banen
]
...
to create-cars
  ; kijken of er niet te veel auto's op de weg zijn
  ...
  if number-of-cars > count road-patches [
    ...
    create-turtles number-of-cars [
      set color blue
      move-to one-of free-space road-patches
...
to-report free-space [road-patches]
  let this-car self
...
to create-road
  ...
  ask patches with [ abs pycor <= number-of-lanes] [
    set pcolor grey - 2.5 + random-float 0.1
  ]
end

to go
  ask turtles with [ ycor != target-lane ] [ move-to-lane ]
  ask turtles with [ patience <= 0 ] [ choose-lanes ]
  ask turtles [
...
to-report number-of-lanes
  ; kan je aangeven hoeveel wegen je wilt.
  report 2
end
@#$#@#$#@
GRAPHICS-WINDOW
